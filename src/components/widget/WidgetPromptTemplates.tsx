...
import { FileText, Plus, Tag, Edit, Trash, Copy, MessageSquare } from "lucide-react";
...
export const WidgetPromptTemplates = () => {
  ...
  const { data: templates = [], isLoading } = useQuery({
    queryKey: ['promptTemplates', 'widgets'],
    queryFn: () => fetchPromptTemplates().then(templates => 
      templates.filter(t => t.category === 'Widget')
    )
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Widget prompt template created successfully");
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['promptTemplates'] });
      toast.success("Widget prompt template updated successfully");
      setShowTemplateDialog(false);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['promptTemplates'] });
      toast.success("Widget prompt template deleted successfully");
    },
    onError: (error) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">Widget Prompt Templates</h3>
      ...
      {isLoading ? (
        ...
        <Card className="bg-muted/50">
          <CardContent className="py-6 flex flex-col items-center text-center">
            ...
            <p className="font-medium">No widget templates found</p>
            <p className="text-sm text-muted-foreground mb-4">
              Create templates for your chat widget responses
            </p>
      ...
      <Dialog open={showTemplateDialog} onOpenChange={setShowTemplateDialog}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{editingTemplate ? "Edit Widget Prompt Template" : "Create Widget Prompt Template"}</DialogTitle>
            <DialogDescription>
              {editingTemplate 
                ? "Modify your existing widget prompt template"
                : "Create a new widget prompt template for consistent chat responses"}
            </DialogDescription>
          </DialogHeader>
